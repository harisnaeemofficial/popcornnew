	...
	home:{
		...
		catalog:{
			...
			appendItem:function(movie, section, i){
				...
				var imdb = "";
				if(section == "tv") {
					imdb = (movie.imdb.replace('tt',"") + "-1-1");
				} else {
					imdb = (movie.imdb.replace('tt',""));
				}

				$('<a href="/'+ slugify(movie.title + ' '+ movie.year) +'.html?imdb=' + imdb +'" data-section="'+section+'"  data-movie_id="'+movie.imdb+'"  id="movie-'+movie.imdb+'" style="width:' + ui.home.catalog.item_width + 'px;height:' + ui.home.catalog.item_height + 'px;transition-delay: 0s, '+(i/23)+'s;" class="movie unloaded p' + ui.home.catalog.page + '">'+html+'</a>').appendTo('#catalog_scroller')
/*				.click(function(e){
